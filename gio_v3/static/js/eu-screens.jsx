...
function AnchorsTodayCard({ anchors, eurythmiaCard, pillarMeta, onLog }) {
  ...
  return (
    <div style={{background:C.card2, border:'1px solid var(--b)', borderRadius:14, padding:'12px 14px', marginBottom:14}}>
      <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:9}}>
        <div style={{width:20, height:20, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center',
          background:'var(--gold-bg)', color:C.gold, flexShrink:0}}>
          <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
            <path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/>
          </svg>
        </div>
        <span style={{fontFamily:'DM Sans,sans-serif', fontSize:11, fontWeight:600, color:C.text}}>Anclas de hoy</span>
        <span style={{fontFamily:'DM Sans,sans-serif', fontSize:9, color:C.textMuted, marginLeft:'auto'}}>{total}</span>
      </div>
      <div style={{display:'flex', flexDirection:'column', gap:7}}>
        {eurythmiaCard && (
          <div style={{display:'flex', alignItems:'center', gap:9}}>
            <div style={{width:18, height:18, borderRadius:6, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center',
              background:'rgba(232,64,148,0.10)', color:'#e84094'}}>
              <IconMusic size={10}/>
            </div>
            <span style={{flex:1, fontFamily:'DM Sans,sans-serif', fontSize:11.5,
              color: eurythmiaCard.done ? C.textMuted : C.textSub}}>{eurythmiaCard.title}</span>
            <span style={{fontFamily:'DM Sans,sans-serif', fontSize:9, padding:'2px 7px', borderRadius:100, flexShrink:0,
              background: eurythmiaCard.done ? 'rgba(74,222,128,0.12)' : C.card,
              color: eurythmiaCard.done ? '#4ade80' : C.textMuted}}>{eurythmiaCard.badge}</span>
          </div>
        )}
        {anchors.map(act => {
          ...
          return (
            <div key={act.key} {...clickableProps(() => onLog(act.key))} style={{display:'flex', alignItems:'center', gap:9, cursor:'pointer'}}>
              <div style={{width:16, height:16, borderRadius:5, flexShrink:0,
                ...
                {act.done && (
                  <svg width={9} height={9} viewBox="0 0 11 11">
                    <polyline points="2,5.5 4.5,8.5 9,2.5" stroke="#fff" strokeWidth={1.8} fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                )}
              </div>
              <span style={{flex:1, fontFamily:'DM Sans,sans-serif', fontSize:11.5,
                color: act.done ? C.textMuted : C.textSub, textDecoration: act.done ? 'line-through' : 'none'}}>{act.label}</span>
              <span style={{fontFamily:'DM Sans,sans-serif', fontSize:9, color:col, flexShrink:0}}>+{act.pts} XP</span>
            </div>
          );
        })}
      </div>
